...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'order.create': {
    roles: ['pizza-shop-owner', 'store-manager', 'kitchen-staff'],
    key: 'new-customer-order',
    tenantPath: ['pizza_shop', 'user', 'order'],
    userPath: [],
  },
  'order.update': {
    roles: ['pizza-shop-owner', 'store-manager', 'delivery-driver', 'customer'],
    key: 'order-status-update',
    tenantPath: ['pizza_shop', 'user', 'order'],
    userPath: [],
  },
  'offer.create': {
    roles: ['pizza-shop-owner', 'store-manager', 'customer'],
    key: 'new-offer-created',
    tenantPath: ['pizza_shop', 'user', 'offer'],
    userPath: [],
  },
};

const ownerRoles: string[] = ['pizza-shop-owner'];
const customerRoles: string[] = ['customer'];
const tenantRoles: string[] = ['store-manager', 'delivery-driver', 'kitchen-staff', 'business-owner'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.pizza_shop.findFirst({
