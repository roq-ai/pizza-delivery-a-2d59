...
import { getPizzaShops } from 'apiSdk/pizza-shops';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['pizza-shop-owner', 'store-manager', 'delivery-driver', 'kitchen-staff', 'business-owner'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<PizzaShopInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/pizza-shops` : null),
    () => getPizzaShops({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Pizza Shops',
      path: isTenantUser ? `/pizza-shops/view/${data?.[0]?.id}` : '/pizza-shops',
      entity: 'pizza_shop',
      service: AccessServiceEnum.PROJECT,
      icon: FiShoppingBag,
    },
    { name: 'Deliveries', path: '/deliveries', entity: 'delivery', service: AccessServiceEnum.PROJECT, icon: FiTruck },
    { name: 'Meals', path: '/meals', entity: 'meal', service: AccessServiceEnum.PROJECT, icon: FiCoffee },
