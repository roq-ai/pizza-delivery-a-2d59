...
export default RoqAuth({
  hooks: {
    ...
    onRegisterSuccess: async ({ user }) => {
      ...
      if (user.roles.data.some((role) => ['Pizza Shop Owner'].includes(role.name))) {
        const tenant = await roqClient.asSuperAdmin().tenant({ id: user.tenantId });
        const { name: tenantName } = tenant.tenant ?? {};
        seedNumber = await prisma.pizza_shop.count();
        const mainEntity = await prisma.pizza_shop.create({
          data: {
            tenant_id: user.tenantId,
            user_id: owner.id,
            name: tenantName,
