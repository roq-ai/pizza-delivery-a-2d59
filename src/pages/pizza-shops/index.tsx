...
import { getPizzaShops, deletePizzaShopById } from 'apiSdk/pizza-shops';
import { PizzaShopInterface } from 'interfaces/pizza-shop';

type ColumnType = ColumnDef<PizzaShopInterface, unknown>;

interface PizzaShopListPageProps {
...
export function PizzaShopListPage(props: PizzaShopListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<PizzaShopInterface>>(
    () => `/pizza-shops?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: PizzaShopInterface) => {
    if (hasAccess('pizza_shop', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/pizza-shops/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/pizza-shops/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('pizza_shop', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/pizza-shops/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('pizza_shop', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
